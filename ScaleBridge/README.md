...
## Local engineering fork: verified CPU / TorchScript path

This checkout also provides an opt-in TensorRT-free, bounded MuJoCo workflow.
See the [local deployment audit](../docs/SCALEBFM_SIM_CHAIN_20260909.md) for
the existing-environment commands, explicit model/data provenance and results.
The official M actor passed 16 export checks; the height-preserving local
variant completed 48 short-reference physics runs with 47/48 tracking guards
passed, not complete BFM or manipulation acceptance. No real-robot interface was
used. The installation and hardware directions below are upstream instructions,
not evidence that these deployment targets were validated locally.
