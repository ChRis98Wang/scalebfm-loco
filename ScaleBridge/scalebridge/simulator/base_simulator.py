...
class BaseSimulator:
    ...
    def _setup_backbone(self):
        ...
        if (not np.isfinite(self.low_dt) or self.low_dt <= 0 or type(self.decimation) is not int
                or self.decimation < 1 or not np.isclose(self.high_dt, .02, atol=1e-12, rtol=0)):
            raise ValueError("BFM requires a finite 50 Hz policy clock and integer physics decimation")
    ...
    def _setup_asset(self):
        ...
        for names in (sim_end_joint_names, env_end_joint_names, env_end_action_names):
            if len(names) != self.num_joints or len(set(names)) != self.num_joints or set(names) != set(sim_end_joint_names):
                raise ValueError("Simulator/observation/target joint names must be unique matching sets")
        ...
        gains = np.asarray(self.metadata_dict["stiffness"], dtype=np.float32)
        if gains.shape != (self.num_joints,) or not np.isfinite(gains).all() or np.any(gains < 0):
            raise ValueError("Invalid joint-ordered stiffness metadata")
        self.stiffness[self.sim_to_env_joint_idx] = gains
        
        self.damping = np.zeros((len(sim_end_joint_names),), dtype=np.float32)
        gains = np.asarray(self.metadata_dict["damping"], dtype=np.float32)
        if gains.shape != (self.num_joints,) or not np.isfinite(gains).all() or np.any(gains < 0):
            raise ValueError("Invalid joint-ordered damping metadata")
        self.damping[self.sim_to_env_joint_idx] = gains
