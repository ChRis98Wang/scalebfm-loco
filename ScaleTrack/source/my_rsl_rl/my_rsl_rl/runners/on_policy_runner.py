...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device: str = "cpu") -> None:
        ...
        # Opt-in dataset coverage, independent of the expensive evaluation loop.
        self.motion_resample_interval = self.cfg.get("motion_resample_interval", 0)
        self.motion_sampling_strategy = self.cfg.get("motion_sampling_strategy", "legacy")
        self._motion_cohort_sampler = None
        self._pending_motion_cohort = None

    @property
    def motion_cohort_sampler(self):
        return self._motion_cohort_sampler

    def _motion_catalog_hash(self):
        command = self.env.unwrapped.command_manager.get_term(self.command_name)
        # IDs refer to this exact ordering, not merely the number of motions.
        encoded = json.dumps(list(command.motion_names_train), ensure_ascii=False, separators=(",", ":"))
        return hashlib.sha256(encoded.encode("utf-8")).hexdigest()

    def _prepare_motion_cohorts(self):
        """Initialize a training-only curriculum before the first policy action."""
        from my_rsl_rl.utils.motion_cohort import UniformMotionCohortSampler, validate_uniform_probabilities

        interval = self.motion_resample_interval
        if isinstance(interval, bool) or not isinstance(interval, int) or interval < 0:
            raise ValueError("motion_resample_interval must be a nonnegative integer")
        if self.motion_sampling_strategy == "legacy":
            if interval != 0 or self._pending_motion_cohort is not None:
                raise ValueError("legacy requires interval=0 and no resumed coverage curriculum")
            return None  # Preserve the entire original train/reset/RNG path.
        if self.motion_sampling_strategy != "coverage" or interval == 0:
            raise ValueError("strategy must be coverage with a positive motion_resample_interval")
        if self.is_distributed:
            raise ValueError("distributed coverage sampling is unsupported; use single-GPU training")
        if self.eval_during_training:
            raise ValueError("coverage requires separate evaluation, not training-time adaptive evaluation")
        command = self.env.unwrapped.command_manager.get_term(self.command_name)
        if command.is_evaluating or command.has_test_set:
            raise ValueError("coverage training must not load a held-out/evaluation pool")
        if list(command.motion_names) != list(command.motion_names_train):
            raise ValueError("coverage requires the active training motion catalog")
        validate_uniform_probabilities(command.motion_sampling_prob, len(command.motion_names_train))
        sampler = UniformMotionCohortSampler(len(command.motion_names_train), self.env.num_envs, self.cfg["seed"])
        pending = self._pending_motion_cohort
        if pending is not None:
            if pending.get("catalog_sha256") != self._motion_catalog_hash() or pending.get("interval") != interval:
                raise ValueError("resumed coverage catalog or interval differs from checkpoint")
            sampler.load_state_dict(pending["sampler"])
        self._motion_cohort_sampler = sampler
        self._pending_motion_cohort = None
        return self._advance_motion_cohort()

    def _advance_motion_cohort(self):
        """Full reference/robot/history reset at a completed PPO update boundary."""
        from my_rsl_rl.utils.motion_cohort import validate_uniform_probabilities

        command = self.env.unwrapped.command_manager.get_term(self.command_name)
        if command.is_evaluating:
            raise RuntimeError("cannot rotate a training cohort while evaluating")
        validate_uniform_probabilities(command.motion_sampling_prob, len(command.motion_names_train))
        command.motion_ids.copy_(self._motion_cohort_sampler.next_ids())
        command._future_manual_cache.clear()
        command.randomize_next_resampling = True
        # Rollout histories can have been allocated under inference_mode.
        with torch.inference_mode():
            obs, _ = self.env.reset()
            self.alg.policy.reset(torch.ones(self.env.num_envs, dtype=torch.bool, device=self.device))
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False) -> None:
        ...
        cohort_obs = self._prepare_motion_cohorts()
        ...
        # Start learning
        obs = self.env.get_observations().to(self.device) if cohort_obs is None else cohort_obs
        ...
        for it in range(start_iter, tot_iter):
            ...
            # Storage/returns have been consumed by update(); never change the
            # reference halfway through a rollout or reuse pre-reset observations.
            if (self._motion_cohort_sampler is not None and it + 1 < tot_iter
                    and (it - start_iter + 1) % self.motion_resample_interval == 0):
                obs = self._advance_motion_cohort()
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35) -> None:
        ...
        for key, value in getattr(self.alg, "update_diagnostics", {}).items():
            self.writer.add_scalar(f"PPO/{key}", value, locs["it"])
        if getattr(self, "_motion_cohort_sampler", None) is not None:
            for key, value in self._motion_cohort_sampler.snapshot().items():
                self.writer.add_scalar(f"Data/cohort_{key}", value, locs["it"])
    ...
    def save(self, path: str, infos: dict | None = None) -> None:
        ...
        if getattr(self, "_motion_cohort_sampler", None) is not None:
            saved_dict["motion_cohort"] = {
                "catalog_sha256": self._motion_catalog_hash(),
                "interval": self.motion_resample_interval,
                "sampler": self._motion_cohort_sampler.state_dict(),
            }
    ...
    def load(self, path: str, load_optimizer: bool = True, map_location: str | None = None) -> dict:
        ...
        if load_optimizer and resumed_training:
            ...
            # Adaptive PPO writes these scalars back into Adam on every update.
            # Resume from the saved rates, not the newly constructed config.
            self.alg.actor_learning_rate = self.alg.actor_optimizer.param_groups[0]["lr"]
            self.alg.critic_learning_rate = self.alg.critic_optimizer.param_groups[0]["lr"]
        # Load current learning iteration
        if resumed_training:
            self.current_learning_iteration = loaded_dict["iter"]
            # Evaluation/playback may ignore curriculum metadata. A subsequent
            # learn() validates/restores it; robot and rollout state are reset.
            self._pending_motion_cohort = loaded_dict.get("motion_cohort")
