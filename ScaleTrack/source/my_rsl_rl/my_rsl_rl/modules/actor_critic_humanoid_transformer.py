...
class ActorCriticHumanoidTransformer(nn.Module):
    ...
    def get_actor_obs(self, obs: TensorDict, inference: bool = False):
        ...
        if 'mode' in obs:
            ...
            if not inference:
                # Mask out-of-place: playback diagnostics may evaluate multiple
                # modes from one captured observation, and policy inference must
                # never rewrite the environment-owned observation/history tensor.
