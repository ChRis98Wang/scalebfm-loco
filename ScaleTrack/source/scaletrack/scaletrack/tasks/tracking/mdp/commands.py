...
SUPPORTED_MOTION_FORMAT_VERSIONS = {2, 3}
...
def _validate_v3_motion_contract(data, name: str, frame_count: int) -> None:
    """Validate metadata and root references introduced by archive format v3."""
    required_fields = {
        "reference_root_pos",
    ...
    if missing:
        raise ValueError(
            f"Version 3 motion is missing required fields {sorted(missing)}: {name}"
        )

    reference_root_pos = np.asarray(data["reference_root_pos"])
    reference_root_quat = np.asarray(data["reference_root_quat_w"])
    if reference_root_pos.shape != (frame_count, 3):
        raise ValueError(
            "Version 3 reference_root_pos must have shape "
            f"({frame_count}, 3): {name}"
        )
    if reference_root_quat.shape != (frame_count, 4):
        raise ValueError(
            "Version 3 reference_root_quat_w must have shape "
            f"({frame_count}, 4): {name}"
        )
    if not np.isfinite(reference_root_pos).all() or not np.isfinite(
        reference_root_quat
    ).all():
        raise ValueError(f"Version 3 reference root contains NaN or Inf: {name}")
    if not np.allclose(
        np.linalg.norm(reference_root_quat, axis=-1), 1.0, atol=1.0e-3
    ):
        raise ValueError(
            f"Version 3 reference_root_quat_w contains non-unit quaternions: {name}"
        )
    ...
    if (
        body_pos.ndim != 3
        or body_pos.shape[0] != frame_count
        or body_pos.shape[1] < 1
        or body_pos.shape[2] != 3
    ):
        raise ValueError(f"Version 3 body_pos_w has an invalid shape: {name}")
    if (
        body_quat.ndim != 3
        or body_quat.shape[0] != frame_count
        or body_quat.shape[1] < 1
        or body_quat.shape[2] != 4
    ):
        raise ValueError(f"Version 3 body_quat_w has an invalid shape: {name}")
    if not np.isfinite(body_quat).all():
        raise ValueError(f"Version 3 body_quat_w contains NaN or Inf: {name}")
    if not np.allclose(
        np.linalg.norm(body_quat, axis=-1), 1.0, atol=1.0e-3
    ):
        raise ValueError(
            f"Version 3 body_quat_w contains non-unit quaternions: {name}"
        )
    if not np.allclose(
        body_pos[:, 0], reference_root_pos, atol=1.0e-4, rtol=0.0
    ):
        raise ValueError(
            f"Version 3 pelvis positions disagree with reference root: {name}"
        )
    ...
    if float(np.max(quaternion_error)) > 1.0e-4:
        raise ValueError(
            f"Version 3 pelvis rotations disagree with reference root: {name}"
        )

    for field in ("source_sha256", "pipeline_fingerprint"):
        value = np.asarray(data[field]).item()
        if not isinstance(value, str) or SHA256_PATTERN.fullmatch(value) is None:
            raise ValueError(
                f"Version 3 {field} must be a lowercase SHA-256 string: {name}"
            )
...
def load_motion_data_worker(args):
    ...
    try:
        with np.load(motion_path, allow_pickle=False) as data:
            has_format_version = "format_version" in data.files
            has_quaternion_order = "quaternion_order" in data.files
            if has_format_version:
                format_version = np.asarray(data["format_version"]).item()
                if isinstance(format_version, bool) or not isinstance(
                    format_version, int
                ):
                    raise ValueError(
                        f"Motion format version must be an integer scalar: {name}"
                    )
                if format_version not in SUPPORTED_MOTION_FORMAT_VERSIONS:
                    raise ValueError(
                        f"Unsupported motion format version {format_version}: {name}"
                    )
                if not has_quaternion_order:
                    raise ValueError(
                        f"Versioned motion is missing quaternion_order metadata: {name}"
                    )
            if has_quaternion_order:
                quaternion_order = str(np.asarray(data["quaternion_order"]).item())
                if quaternion_order != "wxyz":
                    raise ValueError(
                        "Motion archive quaternion_order must be wxyz, "
                        f"got {quaternion_order!r}: {name}"
                    )

            fps = np.asarray(data["fps"]).item()
            if (
                isinstance(fps, bool)
                ...
                raise ValueError(
                    f"Loaded motion should be exactly 50FPS. "
                    f"An error occurs when loading motion: {name}"
                )
            ...
            if has_format_version and format_version == 3:
                _validate_v3_motion_contract(
                    data, name, data["joint_pos"].shape[0]
                )
            ...
            return {
                ...
                # Motion archives have a stable wxyz contract. Adapt them at the
                # boundary because supported IsaacLab runtimes use either order.
                "body_quat_w": packed_wxyz_to_runtime(
                    data["body_quat_w"][:, body_indexes], RUNTIME_QUATERNION_ORDER
                ),
