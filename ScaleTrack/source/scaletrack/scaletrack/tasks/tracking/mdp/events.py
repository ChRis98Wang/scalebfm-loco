...
def randomize_rigid_body_com(
    ...
    if env_ids is None:
        ...
        env_ids = env_ids.cpu().to(dtype=torch.int32)
    ...
    if not isinstance(coms, torch.Tensor):
        coms = torch.as_tensor(coms, dtype=torch.float32, device="cpu")
    else:
        coms = coms.to(dtype=torch.float32, device="cpu")
    coms = coms.contiguous()

    # Randomize the com in range
    if env_ids.shape[0] == coms.shape[0]:
        coms[:, body_ids, :3] += rand_samples
    else:
        coms_selected = coms[env_ids][:, body_ids, :3].clone()
        coms_selected += rand_samples
        coms = coms.clone()
        coms[env_ids][:, body_ids, :3] = coms_selected

    # Set the new coms
    try:
        asset.root_physx_view.set_coms(coms, env_ids)
    except TypeError:
        # Some IsaacLab/PhysX versions require Warp tensors for view setters.
        try:
            import warp as wp

            coms_wp = wp.from_torch(coms, dtype=wp.float32)
            env_ids_wp = wp.from_torch(env_ids, dtype=wp.int32)
            asset.root_physx_view.set_coms(coms_wp, env_ids_wp)
        except Exception:
            raise

def push_by_setting_velocity_customized(
    env: ManagerBasedEnv,
