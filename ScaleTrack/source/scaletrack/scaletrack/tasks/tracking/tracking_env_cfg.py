...
except ImportError:  # compatibility for newer IsaacLab noise API
    from isaaclab.utils.noise import UniformNoiseCfg as Unoise

import scaletrack.tasks.tracking.mdp as mdp
from scaletrack.utils.offline_terrain import OfflinePlaneTerrainImporter
...
class MySceneCfg(InteractiveSceneCfg):
    ...
    terrain = TerrainImporterCfg(
        class_type=OfflinePlaneTerrainImporter,
...
class BFMTrackingEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        # NOTE:
        # IsaacLab 3.x uses `sim.physics` while older versions expose `sim.physx`.
        # Keep both for compatibility across versions.
        ...
        if hasattr(self.sim, "physics"):
            if self.sim.physics is None:
                try:
                    ...
                    self.sim.physics = PhysxCfg(gpu_max_rigid_patch_count=gpu_max_rigid_patch_count)
                except Exception:
                    raise AttributeError("self.sim.physics is None and PhysxCfg is unavailable.")
            else:
                self.sim.physics.gpu_max_rigid_patch_count = gpu_max_rigid_patch_count
        elif hasattr(self.sim, "physx"):
            self.sim.physx.gpu_max_rigid_patch_count = gpu_max_rigid_patch_count
        # viewer settings
