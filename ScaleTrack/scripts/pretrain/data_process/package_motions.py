...
parser.add_argument(
    ...
    default=50,
    help="Output motion FPS; ScaleTrack currently requires 50.",
)
parser.add_argument("--num_envs", type=positive_int, default=4096, help="Number of parallel environments")
parser.add_argument(
    "--loader_workers",
    type=positive_int,
    default=min(8, os.cpu_count() or 1),
    help="Maximum CPU workers used to load one bounded simulator batch.",
)
parser.add_argument(
    "--motion_file",
    action="append",
    dest="motion_files",
    help="Exact retargeted motion to package; repeat to exclude stale files.",
)
parser.add_argument(
    "--pipeline_fingerprint",
    type=sha256_digest,
    help="Semantic fingerprint supplied by the orchestration wrapper.",
)
...
from scaletrack.utils.motion_archive import (
    relative_archive_path,
    save_motion_archive,
)
from scaletrack.utils.quaternion_compat import (
    detect_runtime_quaternion_order,
    runtime_to_packed_wxyz,
    source_xyzw_to_runtime,
)

RUNTIME_QUATERNION_ORDER = detect_runtime_quaternion_order()


def standalone_pipeline_fingerprint() -> str:
    """Provide deterministic provenance when this script is invoked directly."""
    digest = hashlib.sha256(b"ScaleTrack standalone package pipeline v1\0")
    digest.update(sys.version.encode("utf-8"))
    digest.update(RUNTIME_QUATERNION_ORDER.encode("ascii"))
    for path in (
        Path(__file__),
        Path(__file__).with_name("motion_processing.py"),
        Path(__file__).parents[3]
        / "source/scaletrack/scaletrack/utils/motion_archive.py",
        Path(__file__).parents[3]
        / "source/scaletrack/scaletrack/utils/quaternion_compat.py",
        Path(__file__).parents[3]
        / "source/scaletrack/scaletrack/robots/g1_29dof.py",
    ):
...
PIPELINE_FINGERPRINT = (
    args_cli.pipeline_fingerprint or standalone_pipeline_fingerprint()
)
...
def load_single_motion(input_dir, motion_file, output_dt):
    try:
        ...
        # ScaleRetarget stores root rotations as xyzw. Convert at the simulator
        # boundary instead of assuming a particular IsaacLab release.
        base_rot = source_xyzw_to_runtime(
            torch.from_numpy(motion_dict['root_rot']).float(),
            RUNTIME_QUATERNION_ORDER,
        )
        ...
        # The local angular-velocity helpers below have an explicit wxyz API.
        motion_base_rot_wxyz = runtime_to_packed_wxyz(
            motion_base_rot, RUNTIME_QUATERNION_ORDER
        )
        q_prev, q_next = motion_base_rot_wxyz[:-2], motion_base_rot_wxyz[2:]
        ...
        return {
            ...
            'file_name': relative_archive_path(input_dir, motion_file).with_suffix("").as_posix(),
...
class BatchMotionLoader:
    def __init__(
        ...
        self.motion_files = sorted(
            motion_files,
            key=lambda path: (Path(path).stat().st_size, str(path)),
        )
        ...
        print(
            f"Will stream {self.num_motions} motions in {self.num_batches} batches "
            f"with at most {self.loader_workers} loader workers"
        )

    def get_current_batch(self):
        """Load only the current simulator batch into memory."""
        batch_files = select_motion_batch(
            self.motion_files,
        ...
        worker_count = min(self.loader_workers, len(batch_files))
        batch_motions = joblib.Parallel(n_jobs=worker_count, verbose=10)(
            joblib.delayed(load_single_motion)(
                self.input_dir, motion_file, self.output_dt
            )
            for motion_file in batch_files
        )
        failed_files = [
            path
            for path, motion in zip(batch_files, batch_motions, strict=True)
            if motion is None or motion['output_frames'] <= 2
        ]
        if failed_files:
            raise RuntimeError(
                "Failed to load valid motion data for: "
                + ", ".join(str(path) for path in failed_files)
            )
        batch_motions = sorted(batch_motions, key=lambda motion: motion['output_frames'])
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, joint_names: list[str]):
    ...
    if args_cli.motion_files is None:
        motion_files = glob.glob(
            f"{args_cli.data_dir}/**/*.{args_cli.data_format}", recursive=True
        )
    else:
        motion_files = args_cli.motion_files
        for motion_file in motion_files:
            path = Path(motion_file)
            if not path.is_file() or path.suffix != f".{args_cli.data_format}":
                raise FileNotFoundError(f"Retargeted motion does not exist: {path}")
            relative_archive_path(args_cli.data_dir, path)
    if not motion_files:
        raise FileNotFoundError(
            f"No motion files found in {args_cli.data_dir} with format {args_cli.data_format}"
        )
    ...
    motion_loader = BatchMotionLoader(
        ...
        loader_workers=args_cli.loader_workers,
    )
    ...
    for batch_idx in range(motion_loader.num_batches):
        ...
        for env_idx in range(actual_batch_size):
            ...
            batch_logs[env_idx] = {
                ...
                "source_path": batch_motions[env_idx]['source_path'],
            }
        ...
        while simulation_app.is_running() and len(completed_envs) < actual_batch_size:
            ...
            for env_idx in range(actual_batch_size):
                if frame_counters[env_idx] <= batch_motions[env_idx]['output_frames']:
                    ...
                    body_pos = robot.data.body_pos_w[env_idx, :] - scene.env_origins[env_idx]
                    batch_logs[env_idx]["body_pos_w"].append(body_pos.cpu().numpy().copy())
                    # Keep one stable on-disk format (wxyz), independent of the
                    # active IsaacLab runtime's in-memory convention.
                    packed_body_quat = runtime_to_packed_wxyz(
                        robot.data.body_quat_w[env_idx, :],
                        RUNTIME_QUATERNION_ORDER,
                    )
                    batch_logs[env_idx]["body_quat_w"].append(packed_body_quat.cpu().numpy().copy())
            ...
            for env_idx in range(actual_batch_size):
                if env_idx not in completed_envs and frame_counters[env_idx] >= batch_motions[env_idx]['output_frames']:
                    ...
                    output_path = Path(args_cli.output_dir) / f"{motion_name}.npz"
                    ...
                    # Saved arrays use articulation order, not the source PKL
                    # order. Keep names with the arrays for independent audits.
                    log_data["joint_names"] = np.asarray(robot.joint_names, dtype=str)
                    log_data["body_names"] = np.asarray(robot.body_names, dtype=str)
                    source_motion = batch_motions[env_idx]
                    log_data["reference_root_pos"] = (
                        source_motion["base_pos"].cpu().numpy().copy()
                    )
                    log_data["reference_root_quat_w"] = runtime_to_packed_wxyz(
                        source_motion["base_rot"],
                        RUNTIME_QUATERNION_ORDER,
                    ).cpu().numpy().copy()
                    ...
                    save_motion_archive(
                        output_path,
                        log_data,
                        source_path=Path(batch_logs[env_idx]["source_path"]),
...
def main():
    ...
    print(
        ...
        f"runtime={RUNTIME_QUATERNION_ORDER}, packed=wxyz, ScaleRetarget=xyzw"
    )
    # Load kit helper
