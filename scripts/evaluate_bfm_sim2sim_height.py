#!/usr/bin/env python3
"""Version 2: bounded ScaleBridge tracking, with XY-relative / world-Z control.
...
def tracking_distances(actual, reference, actual_root, reference_forcing):
    ...
    offset = actual_root - reference[0]
    offset = offset.copy()
    offset[2] = 0.  # Height must remain measured world-Z, including local mode.
    control_reference = reference + offset if reference_forcing else reference
...
def one_rollout(policy_path, motion, xml, mode, forcing, max_steps, device, strict_torque_limits):
    ...
    from scalebridge.env.motion_tracking_height import HeightAwareMotionTrackingEnv as MotionTrackingEnv
    ...
    try:
        ...
        return {"mode_index": mode, "reference_forcing": forcing,
            "control_translation": "reference-root-XY-with-world-Z" if forcing else "actual-world-XYZ",
            "position_metric_frame": "XY-aligned local with actual-world height; world drift reported separately" if forcing else "world",
...
def evaluate(policy, packed_root, xml, output, max_steps, modes, forcings, device, strict_torque_limits):
    ...
    report = {"schema": "bfm.sim2sim_tracking_height/2", "result": "ERROR", "training_updates": 0,
              ...
              "local_preserves_world_height": True,
              ...
              "interpretation": "Finite-window reference tracking; local mode removes XY translation only, preserving height. Not arbitrary online targets or manipulation."}
    try:
